...
export default function CholeraData() {
  ...
  const numericalKeys = [
    { key: 'nbcas', label: 'Nombre de cas suspects', icon: '🤒' },
    { key: 'nbdeces', label: 'Nombre de décès (choléra)', icon: '⚰️' },
    { key: 'nbgueris', label: 'Nombre de guéris', icon: '💪' },
    { key: 'nbhospitalises', label: 'Lits disponibles', icon: '🏥' },
  ...
  const handleRowClick = (provinceName) => {
    router.push(`/solidarite/cholera/${encodeURIComponent(provinceName)}`);
  };
  ...
  const exportPDF = async () => {
    ...
    try {
        ...
        doc.setFontSize(16);
        const title2 = "Situation générale du choléra";
        ...
        doc.save('situation_cholera.pdf');
    } catch (error) {
  ...
  return (
    <Layout>
      <Head>
        <title>Données du Choléra – DPASSAH</title>
      </Head>
      ...
      <div className="container">
        <h2 style={{ textAlign: 'center', color: '#143c6d', marginBottom: '2rem' }}>
          Données du Choléra par Province
        </h2>
        ...
        {loading ? (
          ...
          <div style={{ textAlign: 'center', padding: '20px', fontSize: '1.1rem', color: '#555' }}>
            Aucune donnée de choléra à afficher.
          </div>
        ) : (
          <>
            {isLoggedIn && (
