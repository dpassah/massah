...
export default function AidesData() {
  ...
  async function fetchData() {
    ...
    if (error) {
      console.error('Error fetching aides data:', error);
      setRows([]);
    } else {
      setRows(data || []);
    }
  ...
  rows.forEach(row => {
    if (!processedData[row.province]) {
      processedData[row.province] = { reports: [], organismes: new Set() };
      numericalKeys.forEach(nk => {
        processedData[row.province][`total_${nk.key}`] = 0;
      });
    }
    numericalKeys.forEach(nk => {
      processedData[row.province][`total_${nk.key}`] += (parseFloat(row[nk.key]) || 0);
    });
    if(row.organisme) {
        processedData[row.province].organismes.add(row.organisme);
    }
  ...
  const totals = rows.reduce((acc, row) => {
    numericalKeys.forEach(item => {
      acc[item.key] = (acc[item.key] || 0) + (parseFloat(row[item.key]) || 0);
    });
    return acc;
  }, {});

  const handleRowClick = (provinceName) => {
    router.push(`/solidarite/aides/${encodeURIComponent(provinceName)}`);
  };
  ...
  const exportPDF = async () => {
    ...
    try {
        ...
        doc.setFontSize(16);
        const title2 = "Situation générale des aides";
        ...
        const tableRows = provinces.map(provinceName => {
            ...
            numericalKeys.forEach(nk => {
                rowData.push(processedData[provinceName][`total_${nk.key}`].toLocaleString('fr-FR'));
            });
            rowData.push(Array.from(processedData[provinceName].organismes).join(', '));
            return rowData;
        });

        const totalRow = ['Total Général'];
        numericalKeys.forEach(nk => {
            totalRow.push(totals[nk.key].toLocaleString('fr-FR'));
        });
        totalRow.push(''); // Empty cell for organismes total
        ...
        autoTable(doc, {
            ...
            styles: { fontSize: 10, cellPadding: 3, font: 'DejaVuSans' },
        });
  ...
  return (
    <Layout>
      <Head>
        <title>Données des Aides – DPASSAH</title>
      </Head>
      ...
      <div className="container">
        <h2 style={{ textAlign: 'center', color: '#143c6d', marginBottom: '2rem' }}>
          Registre des Aides et Solidarités par Province
        </h2>
        ...
        {loading ? (
          ...
          <div style={{ textAlign: 'center', padding: '20px', fontSize: '1.1rem', color: '#555' }}>
            Aucune aide à afficher pour le moment.
          </div>
        ) : (
          <>
            {isLoggedIn && (
                  ...
                  {provinces.map((provinceName) => (
                    ...
                    >
                      ...
                      {numericalKeys.map(nk => (
                        <td key={nk.key}>
                          {processedData[provinceName][`total_${nk.key}`].toLocaleString('fr-FR')}
                        </td>
                      ))}
                      <td>{Array.from(processedData[provinceName].organismes).join(', ')}</td>
                    </tr>
                  ))}
                  <tr className="total-row">
                    <td>Total Général</td>
                    {numericalKeys.map(nk => (
                      <td key={nk.key}>{totals[nk.key].toLocaleString('fr-FR')}</td>
                    ))}
                    <td></td>
                  </tr>
                </tbody>
